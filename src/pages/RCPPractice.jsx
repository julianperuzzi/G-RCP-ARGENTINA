...
function RCPPractice() {
  ...
  return (
    <div className={`flex flex-col justify-center items-center min-h-screen pb-8 transition-all duration-150 ${flash ? 'bg-white' : 'bg-cyan-950'}`}> {/* Fondo hace flash */}
      <div className="w-full max-w-xl bg-white/10 rounded-2xl shadow-2xl p-8 "> {/* Cuadro fijo */}
        {/* Indicador de Ritmo */}
        <div
          className="w-full h-40 flex items-center justify-center bg-white/20 rounded-xl mb-4"
        >
          <FaHeartbeat
            className={`text-6xl ${flash ? 'text-green-500' : 'text-gray-100/50'} transition-all duration-200`}
          />
        </div>
        {/* Título Principal */}
        <h2
          className="text-3xl text-center text-gray-100 mt-4 font-extralight"
          data-aos="fade-down"
        >
          Práctica de RCP
        </h2>
        ...
        {/* Mensaje Inferior */}
        <div
          className="w-full h-32 flex items-center justify-center mt-8 bg-green-400/30 rounded-xl"
        >
